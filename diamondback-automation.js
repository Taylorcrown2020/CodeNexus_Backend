// ============================================================================
// diamondback-automation.js — Diamondback Coding
// Public scheduling + availability, post-job satisfaction surveys, service-request
// responses, and sales-agreement emailing. Ported from crown-automation.js.
//
...
//
// NOTE: this module and diamondback-portal.js both register
// /api/public/availability and /api/public/schedule. Mount ONE of them, or
// delete those two routes from whichever you mount second — Express uses the
// first match, so a duplicate silently shadows the later one.
// ============================================================================
...
/* ============================================================================
   ...
   Self-contained add-on loaded by server.js with ONE line at the very end:
       ...
       require('./diamondback-automation.js')({ app, pool, transporter, stripe });
...
module.exports = function initDiamondbackAutomation({ app, pool, transporter, stripe }) {
  ...
  const EMAIL_FROM = 'Diamondback Coding <contact@diamondbackcoding.com>';
  const REPLY_TO = 'contact@diamondbackcoding.com';
  const SITE = process.env.PUBLIC_BASE_URL || 'https://diamondbackcoding.com';
...
<table role="presentation" width="600" cellpadding="0" cellspacing="0" style="width:600px;max-width:600px;background:${C.card};border-radius:12px;overflow:hidden;">
  <tr><td style="background:${C.card};padding:28px 40px 20px;border-bottom:1px solid ${C.line};">
    <span style="font-family:${SERIF};font-size:13px;letter-spacing:6px;color:${C.gold};text-transform:uppercase;">&#9819; Diamondback</span>
    <span style="font-family:${FONT};font-size:11px;letter-spacing:3px;color:${C.muted};text-transform:uppercase;"> &nbsp;Web Development</span>
  </td></tr>
  ...
  <tr><td style="background:#111010;padding:24px 40px;border-top:1px solid ${C.line};font-family:${FONT};font-size:12px;line-height:1.7;color:${C.muted};">
    <strong style="color:${C.text};">Diamondback Coding</strong> &middot; Forney &amp; the DFW metro<br>
  ...
  function mailConsultationConfirmed(name, whenStr, service) {
    return shell('Consultation booked', 'We&rsquo;ve got you on the calendar.',
      ...
      'Your Diamondback consultation is confirmed.');
  }
  ...
  function mailServiceScheduled(name, whenStr, service) {
    return shell('Service scheduled', 'Your appointment is booked.',
      ...
      'Your Diamondback service is scheduled.');
  }
  ...
  function mailPaymentReceipt(name, amount, invNo) {
    return shell('Payment received', 'Thank you — payment confirmed.',
      P(`Hi ${esc(name)},`) +
      P(`We&rsquo;ve received your payment${amount ? ` of <strong style="color:${C.gold2}">${esc(amount)}</strong>` : ''}${invNo ? ` for invoice <strong>${esc(invNo)}</strong>` : ''}. Your account is fully paid &mdash; thank you for trusting Diamondback.`) +
      P(`A copy is saved in your client portal. If you need anything else, just reply.`),
      'Your payment to Diamondback Coding is confirmed.');
  }
  function mailSurvey(name, link, service) {
    return shell('How did we do?', 'Your finish is ready — tell us how we did.',
      P(`Hi ${esc(name)},`) +
      P(`Thanks for trusting Diamondback with your ${service ? esc(service) : 'vehicle'}. Your feedback takes about a minute and helps us keep raising the bar.`) +
      ...
      'A quick favor — how did Diamondback do?');
  }
  ...
  function mailServiceRequestReply(name, type, response) {
    return shell('We replied', 'A response to your request.',
      ...
      'Diamondback responded to your service request.');
  }
  function mailSalesAgreement(name, agNo, service, price) {
    return shell('Your agreement', 'Your service agreement is ready.',
      P(`Hi ${esc(name)},`) +
      P(`Attached is your Diamondback Coding service agreement${agNo ? ` (<strong>${esc(agNo)}</strong>)` : ''}${service ? ` for <strong>${esc(service)}</strong>` : ''}${price ? `, total <strong style="color:${C.gold2}">${esc(price)}</strong>` : ''}.`) +
      ...
      'Your Diamondback service agreement is attached.');
  }
  ...
  function buildAgreementPDF(ag) {
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        doc.moveDown(0.2).fillColor('#444').fontSize(10).font('Helvetica').text('Web Development · CRM Implementation · Detailing — Forney & DFW, TX', { align: 'center' });
        ...
        doc.moveDown(2).fontSize(10).fillColor('#666').text('Coating warranties, where applicable, are valid only with recommended maintenance and care. This agreement is subject to Diamondback Coding\u2019s full Terms of Service.', { align: 'left' });
        doc.moveDown(2).fillColor('#111').fontSize(11);
        doc.text('Customer signature: ______________________________     Date: ____________');
        doc.moveDown(1).text('Diamondback representative: ____________________________     Date: ____________');
        doc.end();
      } catch (e) { reject(e); }
    });
  }
  ...
  app.post('/api/public/schedule', async (req, res) => {
    try {
      ...
      if (type === 'service') await sendMail({ to: email, subject: 'Your Diamondback service is scheduled', html: mailServiceScheduled(name, whenStr, service) });
      else await sendMail({ to: email, subject: 'Your Diamondback consultation is confirmed', html: mailConsultationConfirmed(name, whenStr, service) });
  ...
  async function createAndSendSurvey({ leadId, name, email, serviceType, appointmentId }) {
    ...
    await sendMail({ to: email, subject: 'How did we do? — Diamondback Coding', html: mailSurvey(name, link, serviceType) });
    return token;
  }
  ...
  app.post('/api/admin/service-requests/:id/respond', authed(), async (req, res) => {
    try {
      ...
      await sendMail({ to: sr.lead_email, subject: 'Diamondback responded to your service request', html: mailServiceRequestReply(sr.lead_name, sr.service_type, response) });
      res.json({ success: true });
  ...
  app.post('/api/admin/sales-agreements/:id/email', authed(), async (req, res) => {
    try {
      ...
      try {
        ...
        attachments.push({ filename: `Diamondback-Agreement-${(ag.agreement_number || ag.id)}.pdf`, content: pdf, contentType: 'application/pdf' });
      } catch (pdfErr) { console.warn('[CROWN agreement pdf]', pdfErr.message); }
      await sendMail({
        to, subject: 'Your Diamondback Coding service agreement',
  ...
  global.__diamondbackEmailAgreement = async (agreementId) => {
    try {
      ...
      const attachments = [];
      try { const pdf = await buildAgreementPDF(ag); attachments.push({ filename: `Diamondback-Agreement-${(ag.agreement_number || ag.id)}.pdf`, content: pdf, contentType: 'application/pdf' }); } catch (_) {}
      await sendMail({ to, subject: 'Your Diamondback Coding service agreement', html: mailSalesAgreement(ag.customer_name || ag.lead_name, ag.agreement_number, [ag.service_type, ag.package_name].filter(Boolean).join(' — '), ag.price != null ? '$' + Number(ag.price).toFixed(2) : ''), attachments });
  ...
  global.__diamondbackMail = {
    consultationCancelled: (name, email, whenStr) => sendMail({ to: email, subject: 'Your Diamondback consultation was canceled', html: mailCancelled(name, whenStr, 'consultation') }),
    serviceCancelled: (name, email, whenStr) => sendMail({ to: email, subject: 'Your Diamondback service was canceled', html: mailCancelled(name, whenStr, 'service appointment') }),
    paymentReceipt: (name, email, amount, invNo) => sendMail({ to: email, subject: 'Payment received — Diamondback Coding', html: mailPaymentReceipt(name, amount, invNo) }),
  ...
  async function runScheduler() {
    try {
      ...
      for (const a of due.rows) {
        const ok = await sendMail({ to: a.lead_email, subject: 'Your Diamondback appointment is in 48 hours', html: mail48hr(a.lead_name, prettyDate(a.scheduled_time), a.service_type) });
